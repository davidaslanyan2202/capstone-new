...
DEFAULT_JSON_DIR = REPO_ROOT / "data" / "scraped" / "transfermarkt" / "history_json"
...
def main() -> int:
    ...
    write_csv(
        args.interim_dir / METRICS_WITH_DATES,
        metrics_rows,
    ...
    write_csv(args.interim_dir / TRANSFER_EVENTS, transfer_events, transfer_event_fields)
    ...
    write_csv(args.interim_dir / CONTRACT_FEATURES, contract_rows, contract_fields)
    ...
    write_csv(args.processed_dir / ANALYTICS_TABLE, analytics_rows, analytics_fields)
    print(f"Analytics rows: {len(analytics_rows)}")
    print(f"Wrote final table: {args.processed_dir / ANALYTICS_TABLE}")
